...
def load_historical_data_from_year(symbols, year):
    try:
        ...
        period = convert_to_date_object((datetime.today() - timedelta(days=1)).strftime('%Y-%m-%d')) - convert_to_date_object(f"{year}-01-01")
        dates = pd.date_range(end=datetime.today(), periods=period.days).to_pydatetime().tolist()
